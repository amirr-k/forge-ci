...
def scheduling_section(payload: dict) -> str:
    ...
    lines = [
        ...
        "distinct from the `cnba run -j N` local-executor numbers above, and never compared",
        "directly to them.",
        "",
        f"- Workload: `demo/scale-monorepo` — 50 modules, 5 dependency layers, **{task_count} tasks**.",
        "- One task per worker (`CNBA_WORKER_MAX_CONCURRENCY=1`).",
...
def faults_section(payload: dict) -> str:
    ...
    lines = [
        ...
        "task run. CNBA never claims exactly-once execution, only that exactly one result is ever",
...
def speculation_section(payload: dict) -> str:
    ...
    lines = [
        ...
        "crash recovery. Only `cnba.scheduler.speculation.enabled` differs between arms.",
        "",
