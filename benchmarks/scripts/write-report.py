...
- **Shared-library change** — median {shared['stats']['median_ms']:.0f} ms against a
  ...
  stddev of {shared['stats']['stddev_ms']:.0f} ms. A change to the module most others depend on
...
- **Toolchain/config change — slower than a plain full build**, by
  ...
  change that invalidates everything, against {(base_ms / leaf_ms):.1f}× on the ordinary
  single-module change. It is amplified by this workload's small tasks
  (~{base_ms / workload['taskCount']:.0f} ms each) — the overhead is roughly fixed per task, so it
