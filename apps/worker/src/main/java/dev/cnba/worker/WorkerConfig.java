...
public record WorkerConfig(
    ...
    public static WorkerConfig fromEnvironment() {
        return new WorkerConfig(
                URI.create(require("CNBA_CONTROL_PLANE_URL")),
                env("CNBA_WORKER_ID", defaultExternalId()),
                splitCsv(env("CNBA_WORKER_CAPABILITIES", "")),
                Integer.parseInt(env("CNBA_WORKER_MAX_CONCURRENCY", "2")),
                env("CNBA_WORKER_VERSION", "0.1.0-SNAPSHOT"),
                Path.of(env("CNBA_WORKSPACE_ROOT", "/workspace")),
                env("CNBA_SEED_WORKSPACE_FROM", "").isBlank()
                        ? null
                        : Path.of(env("CNBA_SEED_WORKSPACE_FROM", "")),
                env("CNBA_WORKER_DOCKER_IMAGE", "alpine:3.20"),
                env("CNBA_WORKER_DOCKER_CPUS", "1"),
                env("CNBA_WORKER_DOCKER_MEMORY", "512m"),
                env("CNBA_WORKER_DOCKER_VOLUME", "").isBlank()
                        ? null
                        : env("CNBA_WORKER_DOCKER_VOLUME", ""),
                Long.parseLong(env("CNBA_WORKER_POLL_INTERVAL_MS", "1000")));
    }
