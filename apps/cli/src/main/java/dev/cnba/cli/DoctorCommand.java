...
import dev.cnba.core.git.GitWorkspace;
 ...
 */
@Command(name = "doctor", description = "Check that this machine and repository can run CNBA.")
final class DoctorCommand implements Callable<Integer> {
    ...
    private static Check configCheck() {
        try {
            ...
            return Check.ok(
                    "configuration",
                    "cnba.yml: " + workspace.graph().size() + " tasks, no cycles");
        } catch (RuntimeException e) {
