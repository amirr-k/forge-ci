...
import dev.cnba.cache.RemoteCacheUnavailableException;
import dev.cnba.cache.TaskCache;
import dev.cnba.cache.ToolchainFingerprint;
...
final class CacheCoordinator {
    ...
    private static TaskCache buildCache(ProjectWorkspace workspace) {
        ...
        try {
            ...
            System.err.println(
                    "cnba: remote cache unavailable, falling back to local-only ("
                            + e.getMessage()
                            + ")");
            return new TaskCache(workspace.directory());
        }
    ...
    /**
     * Records that a selected task will need to run but hasn't yet (a {@code cnba plan} preview).
     */
