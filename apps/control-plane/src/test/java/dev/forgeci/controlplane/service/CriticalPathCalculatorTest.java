...
class CriticalPathCalculatorTest {
    ...
    void aChainWeighsItsRemainingLength() {
        Map<String, Integer> weights = CriticalPathCalculator.weights(List.of(task("a"), task("b", "a"), task("c", "b")));
    ...
    void aForkTakesItsLongestBranchNotTheSumOfBoth() {
        ...
        List<TaskDefinitionEntity> tasks =
                List.of(task("root"), task("short", "root"), task("long-1", "root"), task("long-2", "long-1"));
    ...
    void aDependencyOutsideTheSelectedSetDoesNotAddWeightToTheTasksInIt() {
        ...
        Map<String, Integer> weights = CriticalPathCalculator.weights(List.of(task("selected", "unaffected:build")));
    ...
    void twoIndependentTasksBothWeighOne() {
        Map<String, Integer> weights = CriticalPathCalculator.weights(List.of(task("alpha"), task("beta")));
    ...
    private static TaskDefinitionEntity task(String name, String... dependsOn) {
        return new TaskDefinitionEntity(
                null, name, List.of(dependsOn), "sha256:" + name, "source changed", List.of("true"), List.of(), List.of(), 60);
    }
}
