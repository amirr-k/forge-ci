...
class PublicDemoSecurityIntegrationTest extends ControlPlaneIntegrationTest {
    ...
    static void demoProperties(DynamicPropertyRegistry registry) {
        registry.add("cnba.demo.repo-path", () -> "../../demo/sample-monorepo");
        registry.add("cnba.demo.workspace-path", () -> demoWorkspace.toString());
        registry.add("cnba.demo.warmup.enabled", () -> "false");
    }
    ...
    void clearGuestLimits() {
        redis.delete("cnba:demo:build-lock");
        redis.keys("cnba:demo:rate:*").forEach(redis::delete);
    }
    ...
    void aSecondGuestBuildFromTheSameClientInsideTheWindowIsRateLimited() {
        ...
        redis.delete(
                "cnba:demo:build-lock"); // isolate the rate limit from the one-build-at-a-time
        // lock
    ...
    void aScenarioIdOutsideTheAllowlistIsRejected() {
        for (String cnbad :
            ...
            ResponseEntity<Map<String, Object>> response =
                    startDemoBuild(Map.of("scenario", cnbad, "workerCount", 1));
