...
/**
 * The six public-demo scenarios (product-and-demo.md#demo-repository), each mapped to the
 * scenario id {@code scripts/apply-scenario} understands and the exact paths that scenario
 * changes — the same paths {@link dev.forgeci.core.plan.PlanBuilder} uses to compute the real
 * affected-task closure, so the UI's affected/unaffected split matches what the mutation actually
 * touched, not a guess.
 */
public enum DemoScenario {
    NO_CHANGE("no-change", Set.of()),
    LEAF_MODULE_CHANGE("leaf-module", Set.of("services/pricing/src/main/java/PriceCalculator.java")),
    ...
    public static DemoScenario fromScriptId(String scriptId) {
        return Stream.of(values())
                ...
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException("unknown demo scenario: " + scriptId));
    }
}
