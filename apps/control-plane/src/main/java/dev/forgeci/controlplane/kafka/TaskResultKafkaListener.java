 ...
 * same idempotent, lease-checked path a duplicate HTTP call would: a report against a task run
 * that already resolved with a matching lease is a no-op, not a re-applied effect.
 */
@Component
public class TaskResultKafkaListener {
    ...
    @KafkaListener(topics = KafkaTopics.TASK_RESULTS, containerFactory = "taskResultListenerContainerFactory")
