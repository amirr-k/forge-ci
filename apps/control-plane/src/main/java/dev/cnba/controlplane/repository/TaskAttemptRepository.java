...
public interface TaskAttemptRepository extends JpaRepository<TaskAttempt, Long> {
    ...
    @Query(
            "select a from TaskAttempt a where a.taskRun.id = :taskRunId "
                    + "and a.state in (dev.cnba.controlplane.domain.TaskRunState.LEASED, "
                    + "dev.cnba.controlplane.domain.TaskRunState.RUNNING) "
    ...
    @Query(
            "select a from TaskAttempt a where a.state in "
                    + "(dev.cnba.controlplane.domain.TaskRunState.LEASED, "
                    + "dev.cnba.controlplane.domain.TaskRunState.RUNNING) "
    ...
    @Query(
            "select a from TaskAttempt a where a.state = "
                    ...
                    + "and a.taskRun.build.state = dev.cnba.controlplane.domain.BuildState.RUNNING "
