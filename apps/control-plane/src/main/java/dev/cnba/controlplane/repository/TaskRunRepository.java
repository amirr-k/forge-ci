...
public interface TaskRunRepository extends JpaRepository<TaskRun, Long> {
     ...
     */
    @Query(
            "select t from TaskRun t where t.state = dev.cnba.controlplane.domain.TaskRunState.READY "
                    + "and t.build.state = dev.cnba.controlplane.domain.BuildState.RUNNING "
     ...
     * ready. Selected by {@code cnba.scheduler.policy=fifo}. Exists so the critical-path policy
     ...
     * cnba.scheduler.policy=critical-path-duration}.
     */
    @Query(
            "select t from TaskRun t where t.state = dev.cnba.controlplane.domain.TaskRunState.READY "
                    + "and t.build.state = dev.cnba.controlplane.domain.BuildState.RUNNING "
    ...
    @Query(
            "select t.taskName, t.startedAt, t.completedAt from TaskRun t "
                    ...
                    + "and t.state = dev.cnba.controlplane.domain.TaskRunState.SUCCEEDED "
    ...
    @Query(
            "select count(t) from TaskRun t where t.build.id = :buildId "
                    + "and t.state not in (dev.cnba.controlplane.domain.TaskRunState.SUCCEEDED, "
                    + "dev.cnba.controlplane.domain.TaskRunState.CACHED)")
