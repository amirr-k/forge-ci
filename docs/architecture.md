...
`apps/control-plane`, `apps/worker`, `libs/cache`, and `libs/protocol` are
declared in `settings.gradle.kts` but empty; they are filled in by later
phases. `libs/core` deliberately has no framework dependency so planning
and execution stay usable from the CLI alone.
...
Nothing is reported as reused. Reuse requires the verified
content-addressed cache that a later phase adds, and reporting a cache hit
that has not been verified would be a lie.
